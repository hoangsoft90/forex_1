...
- **✅ Đã xong (2026-08-18)**: SQL mục 13 (`notification_preferences` + `feature_flags` + seed) đã chạy — 18/18 bảng verify đầy đủ · redeploy edge `parse-mt4` (VERSION 2, smoke test OK) · `migrations-phase2` (`pro_unlocks`) đã chạy.
- **⚠️ Việc đang treo (xem [can_lam.md](../../can_lam.md) ở root — 12 việc phân ưu tiên)**: **user revoke GH token cũ đã lộ trong chat** · cung cấp export MT4 thật để mở gate `INSTANT_AUDIT_ENABLED` (M3 vẫn chạy fallback quiz) · commit + push toàn bộ diff đang treo.
